...
- **Counties/Towns on by default for MA fresh load.** `make.ui.R` — `show.boundaries` now defaults to `ma` (TRUE) when `cfg$boundaries` is NULL (a fresh load with no switch-carried state). Regional fresh load still defaults to FALSE.
- **Boundaries not carried across mode switch.** `make.server.R` — removed `boundaries` from the carry list in the switch observer. Bug: switch from regional (default OFF) → MA carried `?boundaries=false`, overriding the MA default. Fix: don't carry it at all; each mode defaults independently on arrival.
- **Yellow parcel outlines when imagery + no layers.** `parcel.server.R`:
  - Added `parcel.color` reactive: `'yellow'` when `input$show.basemap == 'USGS.USImagery'` AND both layer radio buttons are empty; `'purple'` otherwise.
  - `draw.parcels` uses `isolate(parcel.color())` — prevents the viewport observer from taking a spurious dependency on the color reactive.
  - Added `observeEvent(parcel.color(), ignoreInit = TRUE)`: clears the 'parcels' group and redraws all drawn parcels from `parcel.store` with the new color. 'selected' highlight group unaffected. `groupOptions(zoomLevels)` re-applied after redraw.
- **POS SVG crosshatch fill.** Replaces the faint green wash (fillOpacity=0.10) with SVG crosshatch lines — visible on all monitors, monitor-agnostic.
  - `www/pos-hatch.js` — new Shiny custom message handler `applyPosHatch`: injects a 10×10px `<pattern id="posHatch">` into the Leaflet SVG overlay (once; survives pan/zoom), then iterates `widget.layerManager._byGroup['pos']` and sets `fill="url(#posHatch)" fill-opacity="1"` on each `layer._path`. 150ms setTimeout gives Leaflet time to finish rendering before we touch the paths.
  - `make.ui.R` — loads `pos-hatch.js` via `tags$script(src='pos-hatch.js')`.
  - `pos.server.R` — `fillOpacity` changed from 0.10 to 0 (pattern provides the visual; avoids green-flash before JS fires); `weight` reduced from 5 to 2 per Ethan's thinner-border request; `session$sendCustomMessage('applyPosHatch', list())` called after each `addPolygons`.
